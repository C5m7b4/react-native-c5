...
class LineChart<T> extends BasicChart<T, LineChartProps> {
  ...
  render_line_circles(
    ...
    return this.state.data.map((item, index) => {
      ...
      const y = (yMax - itemValue) * (y_gap / y_value_gap) + this.state.margin;
      return (
  ...
  getDPath(curve: boolean) {
    ...
    this.state.data.map((item, index) => {
      let x = this.state.margin * 2 + x_gap * index;
      let y =
  ...
  render_line(lineStrokeWidth: number, lineStroke: string, curve: boolean) {
    const { dPath } = this.getDPath(curve);
    return (
      <Path
        d={dPath}
  ...
  render_shadow(curve: boolean) {
    let { dPath, firstX, firstY, lastX, lastY } = this.getDPath(curve);
    const yOrigin = this.state.containerHeight - this.state.margin;
    const x_split = (firstX - this.state.margin) / 4;
    const y_split = (firstY - yOrigin) / 2;

    const end_x_split =
      (this.state.containerWidth - this.state.margin - lastX) / 4;
    const end_y_split =
